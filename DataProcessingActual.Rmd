...
dt[dt == "NaN" | dt == "-Inf"] <- NA
print(dt)
# unique(dt$ALIEN_SAUCERS_SACTMIN)
# unique(dt$WAITTIME[dt$TYPE == "CLOSED"])
# dt[DATE == "06-30" & TIME == as.ITime("13:40:00")]
```
...
removeOutliers <- function(x) {
  ...
  # get upper limit
  upper <- q["75%"]
  # produce T/F if x is in range
  q <- x >= lower & x <= upper
  # if q is.na, change it to true, else keep q
  q <- ifelse(is.na(q), TRUE, q)
  q <- ifelse(!is.na(q) & !q, NA, q)
  return(x[q])
}
...
dt <- dt[, (cols) :=  shift(.SD, type = "lead"), by = .(DATE), .SDcols = cols]
```
...
unique(dt$ALIEN_SAUCERS_CLOSED)
